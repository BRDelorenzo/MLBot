...
def _setup_mock_provider(mock_call_llm):
    """Configura o mock do call_llm e registra um provider fake."""
    from app.services.ai_enrichment import set_provider_config

    set_provider_config("anthropic", api_key="test-key-12345678901234", model="claude-sonnet-4-20250514")
    mock_call_llm.return_value = _mock_claude_response()


@patch("app.services.ai_enrichment.call_llm")
def test_ai_enrich_product(mock_call_llm, client):
    _setup_mock_provider(mock_call_llm)
...
@patch("app.services.ai_enrichment.call_llm")
def test_ai_enrich_updates_product(mock_call_llm, client):
    _setup_mock_provider(mock_call_llm)
...
@patch("app.services.ai_enrichment.call_llm")
def test_ai_enrich_with_kb(mock_call_llm, client, db):
    """Quando o OEM está na KB, source deve conter 'kb+'."""
    _setup_mock_provider(mock_call_llm)

    from app.models import KBDocument, KBDocumentStatus, KBEntry

    doc = KBDocument(filename="test.pdf", storage_path="/tmp/test.pdf", status=KBDocumentStatus.processed)
    db.add(doc)
    db.flush()
    db.add(KBEntry(
        document_id=doc.id,
...
def test_ai_enrich_no_api_key(client):
    ...
    assert "API Key" in resp.json()["detail"] or "configurada" in resp.json()["detail"]
...
@patch("app.services.ai_enrichment.call_llm")
def test_ai_enrich_replaces_previous_data(mock_call_llm, client):
...
@patch("app.services.ai_enrichment.call_llm")
def test_ai_enrich_with_specific_provider(mock_call_llm, client):
    """Deve aceitar provider específico via query param."""
    from app.services.ai_enrichment import set_provider_config

    set_provider_config("openai", api_key="sk-test-key-12345678901234", model="gpt-4o-mini")
    mock_call_llm.return_value = _mock_claude_response()
